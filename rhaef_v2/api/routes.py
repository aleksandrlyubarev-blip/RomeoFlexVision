...
def build_stats(services: APIServices) -> StatsResponse:
    return StatsResponse(request_id=str(uuid4()), **services.model_router.get_stats())


def create_api_router(services: APIServices | None = None) -> APIRouter:
    svc = services or APIServices(model_router=ModelRouter(client=_fake_completion_client), policy_engine=FrictionPolicyEngine(), idempotency_cache={})
    router = APIRouter()
