...
from rhaef_v2.core.errors import ErrorCode
...
def create_execution_store(settings: RuntimeSettings):
    if settings.storage_backend == "sqlite":
        return SQLiteExecutionRepository(settings.sqlite_path)
...
async def execute_run(payload: RunRequest, services: APIServices) -> RunResponse | APIError:
    ...
    if not payload.messages:
        err = APIError(code=ErrorCode.EMPTY_MESSAGES.value, message="messages must not be empty", request_id=payload.request_id)
    ...
    try:
        ...
        err = APIError(code=ErrorCode.MODEL_ROUTING_FAILED.value, message="model routing failed", request_id=payload.request_id)
        services.idempotency_cache[payload.request_id] = err
        services.execution_store.add_event(ExecutionEvent(request_id=payload.request_id, event="run_failed", payload={"code": err.code}))
        return err
...
def create_api_router(services: APIServices | None = None) -> APIRouter:
    if services is None:
        ...
        services = APIServices(
            model_router=ModelRouter(client=_fake_completion_client, settings=settings),
            policy_engine=FrictionPolicyEngine(settings=settings),
            idempotency_cache={},
            execution_store=create_execution_store(settings),
        )
    router = APIRouter()

    @router.post('/run', response_model=RunResponse)
    async def run_route(payload: RunRequest) -> RunResponse | APIError:
        return await execute_run(payload, services)

    @router.get('/stats')
    async def stats() -> StatsResponse:
        return build_stats(services)

    @router.get('/runs/{request_id}')
    async def get_run(request_id: str) -> ExecutionRecordResponse | APIError:
        record = services.execution_store.get_record(request_id)
        if record is None:
            return APIError(code=ErrorCode.RUN_NOT_FOUND.value, message='run not found', request_id=request_id)
    ...
    async def get_timeline(request_id: str) -> list[TimelineEvent]:
        return [TimelineEvent(**e.__dict__) for e in services.execution_store.get_timeline(request_id)]
