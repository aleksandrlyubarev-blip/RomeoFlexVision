  ...
  return (
    <aside className="w-14 lg:w-56 bg-bg-secondary border-r border-border-subtle flex flex-col h-screen sticky top-0 shrink-0">
      ...
      <div className="h-14 flex items-center px-3 lg:px-4 border-b border-border-subtle gap-2.5">
        <div className="w-7 h-7 flex items-center justify-center text-accent-blue text-lg shrink-0"
      ...
      <nav className="flex-1 py-4 flex flex-col gap-1 px-2">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              key={item.view}
              onClick={() => !locked && onNavigate(item.view)}
              className={`
            ...
            >
              ...
              {locked && <span className="hidden lg:block ml-auto text-xs opacity-50">🔒</span>}
            </button>
          );
        })}
      </nav>
      ...
      <div className="p-2 border-t border-border-subtle">
        <div className="flex items-center gap-2 px-2 py-2">
          <div className="w-6 h-6 rounded-full bg-bg-card border border-border-subtle flex items-center justify-center text-xs text-text-muted shrink-0">
            {isAuthenticated ? '○' : '?'}
          </div>
          <div className="hidden lg:block min-w-0">
            <div className="text-xs text-text-secondary truncate">
              {isAuthenticated ? 'Оператор' : 'Гость'}
            </div>
          </div>
        </div>
      </div>
