  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="px-6 lg:px-8 py-6 max-w-2xl space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-xl font-semibold text-text-primary">Профиль оператора</h1>
          <p className="text-xs text-text-muted mt-0.5">Управление аккаунтом и настройки платформы</p>
        </div>
        ...
        <div className="glass-panel p-6 flex items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-base font-medium text-text-primary truncate">{user?.email ?? 'Демо-пользователь'}</div>
            <div className="text-xs text-text-muted mt-0.5">Оператор · Зарегистрирован: {createdDate}</div>
        ...
        {/* Task stats */}
        <div>
          <div className="text-xs text-text-muted uppercase tracking-widest mb-3">Статистика задач</div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'Всего', value: stats.total, color: 'text-text-primary' },
              { label: 'Завершено', value: stats.completed, color: 'text-accent-cyan' },
              { label: 'Выполняется', value: stats.running, color: 'text-accent-blue' },
              { label: 'Ошибок', value: stats.errors, color: 'text-signal-alert' },
            ].map(s => (
        ...
        <div className="glass-panel p-5 space-y-3">
          <div className="text-xs text-text-muted uppercase tracking-widest mb-1">Информация об аккаунте</div>
          ...
          <div className="flex items-center justify-between py-2">
            <span className="text-sm text-text-secondary">Провайдер</span>
        ...
        {!isSupabaseConfigured && (
          <div className="glass-panel p-5 border border-signal-warning border-opacity-25">
            <div className="text-sm font-medium text-signal-warning mb-2">Supabase не настроен</div>
            <div className="text-xs text-text-secondary leading-relaxed mb-3">
              Для сохранения данных между сессиями добавьте файл <code className="bg-bg-card px-1 rounded text-text-primary">.env</code> в корень проекта:
            </div>
...
VITE_SUPABASE_ANON_KEY=eyJhbGc...`}
            ...
            <div className="text-xs text-text-muted mt-2">
              Затем запустите SQL-миграцию: <code className="text-text-secondary">supabase/migrations/001_initial.sql</code>
            </div>
          </div>
        )}
      </div>
